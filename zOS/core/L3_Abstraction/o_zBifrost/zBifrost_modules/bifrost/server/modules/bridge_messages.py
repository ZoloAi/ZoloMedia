            ...
            if isinstance(result, types.GeneratorType):
                ...
                try:
                    ...
                    for chunk_keys, is_gate, gate_value in result:
                        ...
                        # NEW Phase 1: Inject buffered event data back into zDisplay directives
                        ...
                        if buffered_events:
                            ...
                            for event in buffered_events:
                                if isinstance(event, dict) and 'display_event' in event:
                                    ...
                                    # Find matching zDisplay directives in chunk_data and inject the data
                                    for key, value in chunk_data.items():
                                        if isinstance(value, dict) and 'zDisplay' in value:
                                            zdisplay = value['zDisplay']
                                            if isinstance(zdisplay, dict) and zdisplay.get('event') == event_name:
                                                # Merge event data into the zDisplay directive
                                                chunk_data[key]['zDisplay'].update(event_data)
                                                self.logger.info(f"[MessageHandler] ✅ Injected {event_name} data into {key}")
